...
const source = await readFile(resolve(process.cwd(), "contracts/SignalLantern.sol"), "utf8");
const input = {
  language: "Solidity",
  sources: { "SignalLantern.sol": { content: source } },
  settings: { outputSelection: { "*": { "*": ["abi", "evm.bytecode.object"] } } },
};
const output = JSON.parse(solc.compile(JSON.stringify(input)));
const contract = output.contracts?.["SignalLantern.sol"]?.SignalLantern;
...
console.log(`Deploying SignalLantern from ${account.address} on Base...`);
...
console.log("Set NEXT_PUBLIC_SIGNAL_LANTERN_CONTRACT_ADDRESS to this contract address in Vercel.");
